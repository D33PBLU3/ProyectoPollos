        ...
        private void actualizarTotal()
        {
           ...
           Decimal total = 0;
            ...
            for (int i = 0; i < gridProductos.RowCount - 1; i++)
            {
                ...
                labelTotal.Text = Convert.ToString(venta.totalVenta);
            }
        }
...
//            Producto producto = query.buscarProducto(idProducto);
        ...
        private void numPedido_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
                {
                    venta.idPedidos = Convert.ToInt32(numPedido.Text);
                    venta = query.buscarPedido(venta.idPedidos);
                    c = query.BuscarClienteId(venta.idCliente);
        ...
        private void numPedido_KeyDown_1(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
            {
                ...
                if (venta != null)
                {
                    ...
                    txtComen.Text = venta.comentarios;
                    lbFecha.Text = Convert.ToString(venta.fechaPedido);
                    c = query.BuscarClienteId(venta.idCliente);
                    if (c != null)
                    {
                        txtNombreCliente.Text = c.nombre;
                    ...
                    detalle = query.buscarDetalle(venta.idPedidos);
                    if (detalle != null)
                    {
        ...
        private void textBox1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
            {
                try
                {
                    total = Convert.ToDecimal(pago.Text) - Convert.ToDecimal(labelTotal.Text);
                }
                catch
                {
                    ;
